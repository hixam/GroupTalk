...
public class UserDAOImpl implements UserDAO {
    ...
    public User createUser(String loginid, String password) throws SQLException, UserAlreadyExistsException {
        ...
        String id = null;
        try {
            User user = getUserByLoginid(loginid); //Mira si existe un usuario con ese loginid. Si es así lanza excepcion
            if (user != null)
                throw new UserAlreadyExistsException();

            connection = Database.getConnection();

            stmt = connection.prepareStatement(UserDAOQuery.UUID); //Crea el id único
