...
public class TopicResource {
    ...
    @Path("/tema/{idtema}")
    @DELETE
    public void deleteTopic(@PathParam("idtema") String idtema) { //Eliminar un sting
        String userid = securityContext.getUserPrincipal().getName(); //Obtiene el nombre del user a partir del token
        TopicDAO topicDAO = new TopicDAOImpl();
        try {
            String ownerid = topicDAO.getTopicById(idtema).getAutor_tema(); //Propietario del mensaje
